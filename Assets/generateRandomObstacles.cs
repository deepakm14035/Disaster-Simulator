...
namespace Exodrifter.UnityPython.Examples
{
	public class generateRandomObstacles : MonoBehaviour {
		...
		void Awake () {
			...
			Debug.Log ("mainmenu");
			if (t != null) {
				if (t.GetComponent<mainmenucontroller> ().mode==0)
					GetComponent<PythonUnityHelloWorld> ().enabled = true;
				...
				for (int i = boats.Length; i < noofboats; i++) {
					...
					if (t.GetComponent<mainmenucontroller> ().mode==1||t.GetComponent<mainmenucontroller> ().mode==2) {
						...
						newBoat.GetComponent<obstacle2> ().sea=seaLevel;
							...
							newBoat.GetComponent<obstacle2> ().client=GameObject.Find("serverclient");
							newBoat.GetComponent<obstacle2> ().sea=seaLevel;
				...
				for (int i = people.Length; i < noofpeople; i++) {
					...
					if (t.GetComponent<mainmenucontroller> ().mode==1) {
							t1.GetComponent<moveObjectInWater2> ().enabled = false;
							t1.GetComponent<moveObjectInWater> ().enabled=true;
							t1.GetComponent<moveObjectInWater> ().sea = seaLevel;
